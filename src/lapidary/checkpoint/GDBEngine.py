...
class GDBEngine:
    ...
    def __init__(self,
        ...
        if len(self.args) > 0 and self.args[-1].startswith('<'):
            self.input = self.args[-1][2:]
            self.args = aelf.args[:-1]
        else:
            self.input = None
        print(self.args)
    ...
    def _create_gem5_checkpoint(self, chkpt_dir, debug_mode):
        self.chk_out_dir = chkpt_dir
        chk_loc = self.chk_out_dir / 'cpt.{}'.format(self.chk_num)
        if chk_loc.exists():
    ...
    def _run_base(self, debug_mode):
        ...
        if self.input:
            gdb.execute('run {} < {}'.format(self.args, self.input))
        else:
    ...
    def _try_create_checkpoint(self, chkpt_dir, debug_mode):
        '''
        ...
        if self._can_create_valid_checkpoint():
            ...
            self._create_gem5_checkpoint(chkpt_dir, debug_mode)


    def run_time(self, sec_between_chk, chkpt_dir, max_iter, keyframes, debug_mode):
        '''
        ...
        while max_iter < 0 or self.chk_num < max_iter:
            try:
                ...
                proc.join()
                self._try_create_checkpoint(chkpt_dir, debug_mode)
    ...
    def run_inst(self, insts_between_chk, max_iter, keyframes, debug_mode):
        ...
        f = open(self.binary + '-inst.txt', 'w')

        while max_iter < 0 or self.chk_num < max_iter:
            try:
                gdb.execute('stepi {}'.format(insts_between_chk))
                cntr = cntr + insts_between_chk
