...
def add_arguments(parser):
    ...
    parser.add_argument('--uri', help='Run a benchmark (URI method)', default=False, action='store_true')
    parser.add_argument('--uri-cwd', default=None, help='Current Working Directory (URI method)')
    parser.add_argument('--uri-cmd', default=None, help='Binary to Run (URI method)')
    parser.add_argument('--uri-opt', default=None, help='Binary Options (URI method)')
    parser.add_argument('--uri-inp', default=None, help='Input to Binary (URI method)')
...
def main(args):
    ...
    if args.cmd:
        ...
        print("URI Method Selected")

        if args.uri_cwd:
            print('\tCWD:\t{}'.format(args.uri_cwd))
        if args.uri_cmd:
            print('\tCMD:\t{}'.format(args.uri_cmd))
        if args.uri_opt:
            print('\tOPT:\t{}'.format(args.uri_opt))
        if args.uri_inp:
            print('\tINP:\t{}'.format(args.uri_inp))

        mod_bin = modify_binary_ldd(args.config, str(args.uri_cwd) + "/" + str(args.uri_cmd))
        arg_list = [mod_bin] + args.uri_opt.split(" ")
        if args.uri_inp:
            arg_list.append("< " + args.uri_inp)

        directory = os.getcwd() + "/" + args.directory
        os.mkdir(directory)

        if args.uri_cwd:
            os.chdir(args.uri_cwd)
